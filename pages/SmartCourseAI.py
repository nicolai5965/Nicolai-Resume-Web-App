...
def process_course_questions(course_material_qa, course_material, initial_llm_behavior_guidelines_new, max_words=45, llm_model="anthropic", max_iterations=2, test_mode=False):
    ...
    if 'feedback' not in st.session_state:
        st.session_state.feedback = {}
    if 'final_results_list' not in st.session_state:
        st.session_state.final_results_list = []
        ...
        if not question_answered:
            ...
            if submit_button:
                try:
                    ...
                    output_list.append(output)
                    json_output_list.append(output_json)
                    st.session_state.final_results_list.append(final_results)
    ...
    return output_list, json_output_list, st.session_state.final_results_list, iteration_count
...
# Toggle button for the entire Q&A section
if st.button("Toggle Q&A Section"):
    st.session_state.show_qa_section = not st.session_state.show_qa_section

# Display Q&A section if toggled on
if st.session_state.show_qa_section:
    st.write("## Q&A Section")
    ...
    st.write("Click the button above to show the Q&A section.")
